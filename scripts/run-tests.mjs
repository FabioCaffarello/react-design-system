...
testProcess.on("close", (code) => {
  ...
  const hasBrowserError =
    combinedOutput.includes("Browser connection was closed") ||
  ...
  const testMatch = combinedOutput.match(
  ...
  if (testMatch && testsMatch) {
    ...
    const isOnlyBrowserError =
      hasBrowserError && errorCount <= 1 && !hasRealFailures;

    if (
      (testFilesSuccessRate >= 0.99 &&
        testsSuccessRate >= 0.99 &&
        isOnlyBrowserError) ||
