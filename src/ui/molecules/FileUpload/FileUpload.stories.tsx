...
export const MultipleFiles: Story = {
  ...
  parameters: {
    docs: {
      description: {
        story: "Multiple file upload with limit. Try uploading more than 5 files to see the limit in action.",
      },
    },
  },
};
...
export const WithProgress: Story = {
      ...
      <div className="w-full max-w-md space-y-4">
        ...
        {files.length > 0 && (
          <div className="text-sm text-gray-600">
            <p><strong>Files with progress:</strong></p>
            <ul className="list-disc list-inside mt-1">
              {files.map((file) => (
                <li key={file.id}>
                  {file.file.name} - {file.progress !== undefined ? `${file.progress}%` : 'No progress'}
                </li>
              ))}
            </ul>
          </div>
  ...
  parameters: {
    docs: {
      description: {
        story: "File upload with real progress simulation. Upload files to see the progress bars animate.",
      },
    },
  },
};
...
export const WithValidation: Story = {
            ...
            {validFiles.length > 0 && (
              <div className="p-3 bg-green-50 border border-green-200 rounded-md">
                <p className="text-sm text-green-800">
                  ✓ {validFiles.length} file(s) valid: {validFiles.map(f => f.file.name).join(", ")}
                </p>
              </div>
            )}
            {hasErrors && (
  ...
  parameters: {
    docs: {
      description: {
        story: "File upload with real validation. Try uploading files larger than 2MB or wrong file types to see validation errors.",
      },
    },
  },
};
...
export const WithFileRemoval: Story = {
  render: () => {
    ...
    return (
      <div className="w-full max-w-md space-y-4">
        ...
        {files.length > 0 && (
          <div className="text-sm text-gray-600">
            <p><strong>Current files ({files.length}):</strong></p>
            <ul className="list-disc list-inside mt-1">
              {files.map((file) => (
  ...
  parameters: {
    docs: {
      description: {
        story: "File upload with removal functionality. Upload files and click the X button to remove them.",
      },
    },
  },
};
...
export const ImagePreview: Story = {
  render: () => {
    ...
    return (
      <div className="w-full max-w-md space-y-4">
        ...
        {files.length > 0 && (
          <div className="text-sm text-gray-600">
            <p><strong>Uploaded images ({files.length}):</strong></p>
            <div className="mt-2 grid grid-cols-2 gap-2">
              {files.filter(f => f.preview).map((file) => (
  ...
  parameters: {
    docs: {
      description: {
        story: "Image upload with real previews. Upload image files to see thumbnail previews.",
      },
    },
  },
};
...
export const RealWorldUpload: Story = {
            ...
            <div className="space-y-2">
              {files.map((file) => (
                <div key={file.id} className="flex items-center justify-between text-sm">
                  <span className={file.error ? 'text-red-600' : uploaded.includes(file.id) ? 'text-green-600' : 'text-gray-700'}>
                    {file.file.name}
                  </span>
  ...
  parameters: {
    docs: {
      description: {
        story: 'Real-world file upload scenario with progress simulation, validation, and file management.',
      },
    },
  },
};
...
export const DragAndDrop: Story = {
  render: () => {
    ...
    return (
      <div className="w-full max-w-md space-y-4">
        ...
        {files.length > 0 && (
          <div className="text-sm text-gray-600">
            <p><strong>Dropped files ({files.length}):</strong></p>
            <ul className="list-disc list-inside mt-1">
              {files.map((file) => (
                <li key={file.id}>{file.file.name} ({(file.file.size / 1024).toFixed(2)} KB)</li>
              ))}
            </ul>
          </div>
        )}
        <div className="text-xs text-gray-500 p-2 bg-blue-50 rounded">
          💡 Tip: Try dragging files from your file explorer onto the upload area.
        </div>
      </div>
    );
  },
  parameters: {
    docs: {
      description: {
        story: 'Demonstrates drag and drop functionality. Drag files from your file explorer onto the upload area.',
      },
    },
  },
};
...
export const WithEvents: Story = {
    ...
    return (
      <div className="w-full max-w-md space-y-4">
        <p className="text-sm text-gray-600">
          Upload files or remove them. Check the Actions panel to see events being fired.
        </p>
        <FileUpload
          onFilesChange={handleFilesChange}
          onFileRemove={handleFileRemove}
  ...
  parameters: {
    docs: {
      description: {
        story: 'Demonstrates file upload events. Upload or remove files and check the Actions panel to see events being logged.',
      },
    },
  },
};
...
export const DisabledState: Story = {
  ...
  parameters: {
    docs: {
      description: {
        story: 'Disabled state - file upload is disabled and cannot be interacted with.',
      },
    },
  },
};
