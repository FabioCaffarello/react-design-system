...
export const DateRange: Story = {
      ...
      <div className="p-8 space-y-4">
        ...
        {range.start && !range.end && (
          <p className="text-sm text-gray-600">
            Start date selected: {range.start.toLocaleDateString()}. Select end date.
          </p>
        )}
      </div>
    );
  ...
  parameters: {
    docs: {
      description: {
        story: "Date range picker with real calculation. Select start and end dates to see the range and duration calculated.",
      },
    },
  },
};
...
export const WithMinMaxDate: Story = {
    ...
    const handleChange = (value: Date | null) => {
      ...
      if (value) {
        if (value < minDate || value > maxDate) {
          setError(`Date must be between ${minDate.toLocaleDateString()} and ${maxDate.toLocaleDateString()}`);
        } else {
          setError(null);
        }
      } else {
  ...
  parameters: {
    docs: {
      description: {
        story: "Date picker with min/max date validation. Try selecting dates outside the range to see validation.",
      },
    },
  },
};
...
export const WithDisabledDates: Story = {
    ...
    return (
      <div className="p-8 space-y-4">
        ...
        <div className="text-sm space-y-2">
          ...
          <p className="text-gray-500">
            <strong>Disabled dates:</strong> {disabledDates.map(d => d.getDate()).join(", ")} of this month
          </p>
  ...
  parameters: {
    docs: {
      description: {
        story: "Date picker with disabled dates. Try selecting disabled dates (5th, 10th, 15th) to see them blocked.",
      },
    },
  },
};
...
export const DateRangeWithValidation: Story = {
  render: () => {
    ...
    const handleChange = (value: { start: Date | null; end: Date | null } | null) => {
      if (!value) {
      ...
      if (newRange.start && newRange.end) {
        ...
        if (newRange.start < minDate || newRange.end > maxDate) {
          setError(`Dates must be between ${minDate.toLocaleDateString()} and ${maxDate.toLocaleDateString()}`);
          return;
        }
        setError(null);
      }
      ...
      <div className="p-8 space-y-4">
        ...
        {range.start && range.end && !error && (
          <div className="p-4 bg-green-50 border border-green-200 rounded-lg space-y-2">
            ...
            <p className="text-green-700"><strong>Start:</strong> {range.start.toLocaleDateString()}</p>
            <p className="text-green-700"><strong>End:</strong> {range.end.toLocaleDateString()}</p>
            <p className="text-green-700"><strong>Duration:</strong> {calculateDays()} day(s)</p>
          </div>
        )}
        <div className="text-sm text-gray-600">
          <p><strong>Valid range:</strong> {minDate.toLocaleDateString()} - {maxDate.toLocaleDateString()}</p>
        </div>
      </div>
    );
  },
  parameters: {
    docs: {
      description: {
        story: 'Date range picker with validation. Start date must be before end date, and both must be within the valid range.',
      },
    },
  },
};
...
export const KeyboardNavigation: Story = {
  ...
  parameters: {
    docs: {
      description: {
        story: 'Demonstrates keyboard navigation. Use Tab to focus, then navigate the calendar with arrow keys.',
      },
    },
  },
};
...
export const WithEvents: Story = {
  ...
  parameters: {
    docs: {
      description: {
        story: 'Demonstrates date picker events. Select a date and check the Actions panel to see events being logged.',
      },
    },
  },
};
