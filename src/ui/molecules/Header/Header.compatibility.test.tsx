  ...
  describe('Contexts Independentes (TASK-020)', () => {
    it('should have independent contexts - HeaderContext does not interfere with SideNavbarContexts', () => {
      ...
      const HeaderTestComponent = () => {
        ...
        return (
          <div>
            <span data-testid="header-menu-state">{isMobileMenuOpen ? 'open' : 'closed'}</span>
      ...
      expect(screen.getByTestId('header-menu-state')).toHaveTextContent('closed');
    ...
    it('should manage mobile menu state independently - Header and SideNavbar mobile menus are separate', () => {
      ...
      const HeaderMenuComponent = () => {
        const { isMobileMenuOpen, openMobileMenu, closeMobileMenu } = useHeaderContext();
        return (
          <div>
            <span data-testid="header-menu">{isMobileMenuOpen ? 'open' : 'closed'}</span>
            <button onClick={openMobileMenu} data-testid="open-header">Open Header</button>
            <button onClick={closeMobileMenu} data-testid="close-header">Close Header</button>
          </div>
        );
      };
  ...
  describe('Mobile Menus Isolados (TASK-022)', () => {
    ...
    it('should handle both menus open simultaneously', () => {
      ...
      const HeaderMenuTracker = () => {
        ...
        return (
          <div>
            <span data-testid="header-menu-open">{isMobileMenuOpen ? 'yes' : 'no'}</span>
