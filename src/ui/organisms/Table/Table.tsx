...
function TableComponent<T = unknown>(props: TableProps<T>) {
  ...
  return (
    <div className={`bg-white shadow rounded-lg overflow-hidden ${className}`}>
      <TableProvider {...providerProps}>
        {providerProps.filters && providerProps.filters.length > 0 && <TableFilters />}
        <div className="overflow-x-auto">
          <table
            ...
            aria-colcount={providerProps.columns.length + (providerProps.selectable ? 1 : 0) + (providerProps.actions ? 1 : 0)}
          >
            <TableHeader />
            <TableBody />
          </table>
...
function convertSimplifiedToProviderProps<T extends Record<string, unknown>>(props: SimplifiedTableProps<T>): TableProviderProps<T> {
  const {
    columns,
    data,
    loading,
  ...
  if (mode === 'auto') {
    mode = pagination && pagination.total !== undefined && pagination.onPageChange ? 'server' : 'client';
  }
