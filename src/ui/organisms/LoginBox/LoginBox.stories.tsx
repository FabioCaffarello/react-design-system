...
export const WithEvents: StoryObj<typeof LoginBox> = {
  render: () => {
    ...
    const handleSubmit = fn((credentials: { email: string; password: string }) => {
  ...
  play: async ({ canvasElement }) => {
    ...
    await waitFor(() => {
      expect(canvas.getByRole('button', { name: /login|sign in/i })).toBeInTheDocument();
    });
  },
  parameters: {
    docs: {
      description: {
        story: 'Demonstrates login box events. Fill in and submit the form, then check the Actions panel to see events being logged.',
      },
    },
  },
};
