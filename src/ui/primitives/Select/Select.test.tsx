...
describe("Select", () => {
  ...
  it("shows error state", () => {
    const { container } = render(
      <Select error helperText="Error message" options={options} />,
    );
  ...
  describe("Accessibility", () => {
    ...
    it("has aria-describedby when helperText is provided", () => {
      render(
        <Select helperText="Helper text" id="test-select" options={options} />,
      );
      const select = screen.getByRole("combobox");
  ...
  describe("Edge Cases", () => {
    it("handles controlled select", () => {
      const { rerender } = render(
        <Select value="1" options={options} onChange={vi.fn()} />,
      );
