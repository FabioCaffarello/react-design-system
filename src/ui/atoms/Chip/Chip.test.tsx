...
describe('Chip', () => {
  ...
  describe('Accessibility', () => {
    ...
    it('has aria-label when provided', () => {
      render(<Chip onClick={vi.fn()} aria-label="Custom label">Chip</Chip>);
    ...
    it('does not have tabIndex when disabled', () => {
      render(<Chip onClick={vi.fn()} disabled>Disabled</Chip>);
      const chip = screen.getByRole('button');
      expect(chip).not.toHaveAttribute('tabIndex', '0');
    });

    it('respects custom tabIndex', () => {
      render(<Chip onClick={vi.fn()} tabIndex={-1}>Chip</Chip>);
      const chip = screen.getByRole('button');
