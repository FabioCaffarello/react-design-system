...
node scripts/validate-use-client-in-dist.mjs # directive gate: dist/index.{js,cjs} and dist/hooks/index.{js,cjs} start with "use client"; dist/server/index.{js,cjs} must not. Runs in build:validate. Details: .claude/rules/server-entry.md.
