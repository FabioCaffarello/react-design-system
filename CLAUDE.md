...
- Conventional Commits message format — enforced by commitlint in the `commit-msg` hook (`@commitlint/config-conventional`); `release.yml` runs semantic-release, which derives versions and the CHANGELOG from commit types, so a mistyped type misversions the release.
...
npm run test:a11y:baseline # serial axe baseline of record (light + dark, ~11min on local SSD, workers=1) — local-only; CI invokes a11y-serial-baseline.mjs per theme instead (see a11y paragraph below)
...
node scripts/validate-a11y-baseline.mjs [report] [--compare ref] # gate: exits 1 if critical+serious>0 on any theme present in the report, or on errored stories; default report is a11y-baseline.json; --compare asserts story-set parity against the committed baseline (this is how CI calls it)
...
node scripts/validate-docs-sync.mjs # gate: zero phantom npm scripts (every script name a doc mentions exists in package.json) and zero dead relative markdown links, across all .md/.mdx docs; negative references carry a docs-ok: marker on/above the line. Pre-push, CI lint job. Mechanises the two mechanical checks of .claude/rules/docs-sync.md — the semantic ones stay manual.
...
node scripts/derive-brand-secondary.mjs # report: regenerates the --color-brand-secondary-* HEX stops from the ADR-024 OKLCH spec (hue 295) for manual comparison against src/styles/primitives/brand.css; run when retuning the secondary scale. Not gated. See "Brand primitive scales" in .claude/rules/colors.md.
```
...
The a11y gate in `.github/workflows/ci.yml` is two parallel jobs, `a11y-baseline-light` and `a11y-baseline-dark`. Each reuses the `storybook-static/` artifact from `build-storybook`, runs `node scripts/a11y-serial-baseline.mjs --theme <light|dark> --workers 2 --output a11y-baseline-<theme>.json` (NOT the `test:a11y:baseline` npm script, which would rebuild Storybook and measure both themes serially), then validates with `node scripts/validate-a11y-baseline.mjs a11y-baseline-<theme>.json --compare a11y-baseline.json` — the `--compare` guard asserts the fresh report covers the same story set as the committed baseline, killing the partial-DOM false-green. Both jobs are in the `ci-success` aggregator's `needs` list, so they gate merges to `main` via branch protection (behind the UI paths-filter; doc-only PRs skip them). The validator is the actual enforcement mechanism — `parameters.a11y.test: "error"` in `.storybook/preview.tsx` is cosmetic (no `@storybook/addon-vitest` plugin wired into the vitest workspace), see the long comment there.
