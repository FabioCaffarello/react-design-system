...
- `.claude/rules/server-entry.md` is derived from `src/ui/server.ts` (the canonical re-export list), `scripts/lib/server-safe.mjs` (the classification rule and the React client APIs it watches for), `vite.config.server.ts` (the externalised-dep list and the no-banner discipline), and `package.json` `exports`/`build` (the `./server` mapping and the dual-vite-build chain). Adding a server-safe component means landing it under `src/ui/`, adding its concrete-source-file re-export to `server.ts`, AND updating the inventory in both `server-entry.md` and the CLAUDE.md "Server entry" section in the same commit.
- `CLAUDE.md`'s "Server entry" section is derived from the same four sources as `server-entry.md` plus `server-entry.md` itself for the rules pointer. The Commands section's `validate-server-entry.mjs` / `analyze-server-safe.mjs` / `validate-use-client-in-dist.mjs` lines are derived from those scripts' actual behaviour; if the scripts gain a flag or change scope, the one-line gist in Commands updates in the same commit.
