...
- `CLAUDE.md`'s "Public hooks" section is derived from `.claude/rules/hooks.md` (canonical) and from `src/ui/index.ts` (today's set). It restates the one-line gist + a pointer; the rule file owns the detail.
