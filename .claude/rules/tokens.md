...
## Two CSS outputs: compiled `./styles` vs raw `./theme`

The token CSS ships **two** ways. `src/styles/index.css` → `dist/react-design-system.css` (`./styles`) is the Tailwind-**compiled** bundle (resolved `@layer theme`, `@layer properties`, every component utility) — for consumers who don't run Tailwind. `src/styles/tokens.css` → `dist/tokens.css` (`./theme`, issue #234) is the **raw** `@theme` source + light/dark overrides, inlined (never compiled) by `scripts/build-tokens-css.mjs` — for consumers who run Tailwind v4 themselves. When you add or rename a token here, both outputs update on the next `npm run build` automatically (`tokens.css` mirrors `index.css`'s token import chain). The raw-`@theme` contract and its gate live in `.claude/rules/theme-export.md`.
