...
**Caveat de medição — resolvido no full serial (Passo 3):** o paralelo (workers=6, BrowserContext compartilhada) deixou 2 stories com axe-race no Passo 2 — `components/form--with-events` e `components/toast--clear-all`, exatamente componentes interativos onde a11y mais importa. Full serial (workers=1, fresh BrowserContext por story, ~48min wall clock) fechou 0 errored em 852/852: Form clean (0 nodes), Toast 3 nodes color-contrast (incluídos em Family C). Delta vs paralelo: +4 nodes (+3 serious Toast + 1 critical button-name de timing-variance) + 2 outliers novos (heading-order, scrollable-region-focusable). Para refresh futuro: serial é a forma íntegra (~48min); paralelo é OK para diff incremental após anchor fix, sabendo que pode ter race em 1-2 stories interativas. **UPDATE Phase 4 (PR #144):** a race era `workers + SHARED BrowserContext`. O script atual cria `newContext` fresco por story dentro de `runStory` (linha 223) E desabilita o addon-a11y `afterEach` runner via `globals=a11y.manual:!true` URL flag — as duas raízes da race são estruturalmente impossíveis. Re-medição empírica: 28 stress runs (~9.860 execuções) incluindo as configs originais de falha (workers=6 ×3, workers=8 ×3) reportaram ZERO variância nas 2 stories problemáticas. CI agora roda `--workers 2` (casa o runner 2-vCPU; ver `scripts/a11y-serial-baseline.mjs` header). Guard-rail adicional em CI: `validate-a11y-baseline.mjs --compare a11y-baseline.json` reprova falso-verde silencioso (axe lendo DOM parcial e reportando MENOS violações que o baseline committed). A advertência acima "paralelo pode ter race" é **história, não mais avisos vivos**; manter o parágrafo registra o contexto de como o problema foi diagnosticado.
...
## A11y baseline — settle de 200ms é 50% do wall-clock (Lever B, deferido)

**Descoberto em:** Phase 4 timing investigation (PR #144).
**Estado:** o script `a11y-serial-baseline.mjs` aguarda `sb-show-main` + `#storybook-root` com filhos + um `settleMs` fixo de 200ms antes de injetar axe. Medição (856 stories, light, workers=1, M-series): `waitForRender` é 65% do total wall-clock (207s de 317s); destes, 200ms × 856 = 171s = ~50% do total wall-clock é o `waitForTimeout(200)` fixo. `axe.run` em si é 6%.
**Por que importa lembrar:** Lever A (workers=2) entregou o corte principal da Fase 4 (~11min → ~6min por tema, ~5min off PR latency). Se algum dia ~6min ainda doer, o próximo alvo natural é reduzir esse settle. 200ms cortado pra 50ms economizaria ~130s adicionais por tema (~2min) — outro corte significativo, sem mais runners ou paralelismo.
**Por que NÃO atacar agora:** o settle existe pra dar tempo das play functions e estados async estabilizarem antes do axe ler. Reduzir desloca risco pras 47 stories com `play` function — exatamente o caso onde Phase C PR2 testou equivalência axe-direct vs addon-a11y em apenas **5 stories** (não generalizou). Sob `--workers 2` a redução de settle aumentaria contenção em stories ainda em meio a play; isso pode reintroduzir variância silenciosa (falso-verde se axe ler estado-pré-play; falso-positivo se ler estado-meio-mutado). Lever A é seguro porque não toca timing de render; Lever B mexe diretamente nele.
**Gatilho:** se ~6min de a11y baseline ainda for caminho crítico de PR depois de Lever A consolidar (algumas semanas/meses de runs), OU se a suite crescer pra 1500+ stories e o tempo per-theme voltar pra 10min+. Se o sistema entrar em produção e cada minuto de PR feedback importar, tambêm vale.
**Pré-requisitos pra atacar:**

- **(a)** Inventário das 47 stories com `play` function (grep `play:\s*async`). Cada uma é um caso-teste pra B.
- **(b)** Probe similar ao da Phase 4: rodar workers=2 com `settleMs=50` vs `settleMs=200`, 10× cada nas 47 stories interativas. Comparar violation counts (precisa ser idêntico) E tempos. Se alguma story diverge sob 50ms, expandir para C (esperar play completion explicitamente).
- **(c)** Confirmar que `validate-a11y-baseline.mjs --compare` (gua rd-rail de PR #144) pega qualquer divergência antes de aceitar o ganho.

## A11y baseline — esperar play function completion (Lever C, deferido)

**Descoberto em:** Phase 4 timing investigation (PR #144), seguimento de Phase C PR2 Passo 3.
**Estado:** princípio do fix limpo do trade-off de Lever B. Em vez de settle fixo, o script poderia observar `window.__STORYBOOK_PREVIEW__.currentRender.phase === "played"` (ou equivalente) — espera explícita até o lifecycle terminar. Tira o piso de 200ms das stories sem `play` (a maioria) e mantém isolamento correto nas 47 com `play`.
**Por que importa lembrar:** é o fix principled de Lever B. Quando atacar B, atacar B+C juntos provavelmente faz mais sentido — C é o mecanismo, B é o ganho.
**Por que NÃO atacar agora:** mesma razão de B (não precisa hoje, e a validação tem custo). Phase C PR2 prototipou a técnica em 5 stories e funcionou; generalizar pras 47 requer saber qual `phase` é o marcador certo de "render terminado" pra cada categoria de story (com play, sem play, com `useEffect` async, com portais). Não é difícil mas exige medição.
**Briefing de quem atacar:** ler `chunk-W2OC36OK.js` do addon (referenciado no header do `a11y-serial-baseline.mjs`) — ele mostra qual lifecycle phase o addon usa pra rodar axe. Replicar essa exact phase no script fecha qualquer descompasso script-vs-addon.
