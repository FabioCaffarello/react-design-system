...
The Feature Developer agent is engaged when new components, features, or enhancements need to be implemented. It follows Atomic Design principles, uses design tokens, ensures accessibility, writes comprehensive tests, and integrates seamlessly with the existing codebase. This agent is essential for expanding the design system's capabilities.

## Responsibilities

- Implement new features according to specifications and design requirements
- Design clean, maintainable code architecture following Atomic Design
- Integrate features with existing codebase (components, providers, utilities)
- Write comprehensive tests (unit, integration, accessibility, Storybook)
- Ensure WCAG 2.1 AA accessibility compliance
- Use design tokens instead of hardcoded values
- Document components with JSDoc and Storybook stories

## Best Practices

- **Follow Patterns**: Use existing component patterns (compound components, factory pattern, etc.)
- **Atomic Design**: Place components in correct category (atoms/molecules/organisms/templates/patterns)
- **Design Tokens**: Always use design tokens from `src/ui/tokens/`
- **TypeScript Strict**: No `any` types, proper type definitions
- **Accessibility First**: Implement keyboard navigation and ARIA from the start
...
- `src/ui/` — Main source code organized by Atomic Design categories
- `plop-templates/` — Use `npm run plop` to generate component scaffold
- `docs/ARCHITECTURE.md` — Understand component organization
...
- Determine component category (atom/molecule/organism/template/pattern)
- Review similar components for patterns
- Check design tokens needed
...
```bash
npm run plop
# Select component type (Atom/Molecule/Organism)
# Follow prompts to generate files
```

### 3. Implementation Steps

1. **Component Structure**: Create component following existing patterns
2. **TypeScript Types**: Define all props interfaces
3. **Design Tokens**: Use tokens from `src/ui/tokens/`
4. **Styling**: Use TailwindCSS utility classes
5. **Accessibility**: Add ARIA attributes, keyboard navigation
6. **Tests**: Write unit tests alongside implementation
7. **Stories**: Create Storybook stories with examples
...
```typescript
// Example: FlowCanvas.Root, FlowCanvas.Background
...
```typescript
// Use existing factories as reference
// src/ui/tokens/colors.ts - ColorTokenFactory
```

### 5. Integration Points

**Providers**: Use existing context providers (`ThemeProvider`, `AppProvider`)
**Utilities**: Use shared utilities from `src/ui/utils/` and `src/ui/shared/utils/`
**Hooks**: Create custom hooks in `src/ui/hooks/` or component-specific hooks
**Tokens**: Import from `src/ui/tokens/` (colors, spacing, typography, etc.)
...
- Interaction stories with `play` functions
- Accessibility story
...
```typescript
import { getColorClass } from '../../tokens/colors/utils';
...
After feature completion, document:

- **Component Location**: Where it was added
...
- **Follow-up**: Additional features or improvements needed
