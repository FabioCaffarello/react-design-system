...
The Bug Fixer agent is engaged when bugs are reported, tests fail, or runtime errors occur. It systematically analyzes issues, identifies root causes, implements targeted fixes, and ensures fixes are properly tested and documented. This agent is critical for maintaining code quality and preventing regressions.

## Responsibilities

- Analyze bug reports and error messages from GitHub issues, test failures, or runtime errors
- Identify root causes by tracing through component code, hooks, providers, and utilities
- Implement targeted fixes with minimal side effects, following Atomic Design principles
- Test fixes thoroughly using Vitest, Testing Library, and Storybook
- Verify accessibility compliance after fixes (WCAG 2.1 AA)
- Document fixes in commit messages following Conventional Commits

## Best Practices

- **Reproduce First**: Always reproduce the bug in Storybook or test environment before fixing
- **Write Regression Tests**: Add tests that would have caught the bug to prevent future regressions
- **Follow Patterns**: Use existing component patterns, design tokens, and utility functions
- **Check Dependencies**: Verify if bug affects related components (atoms → molecules → organisms)
- **Test Accessibility**: Ensure fixes don't break keyboard navigation or screen reader support
- **Document Thoroughly**: Include clear commit messages and update Storybook stories if needed
...
- `docs/` — Documentation including architecture, accessibility, and testing guides
- `plop-templates/` — Component generation templates (use `npm run plop` to generate)
- `public/` — Static assets for development server
- `scripts/` — Validation and generation scripts (validate-architecture, validate-a11y, etc.)
- `src/ui/` — Main source code organized by Atomic Design (atoms, molecules, organisms, etc.)
- `storybook-static/` — Built Storybook output (generated, don't edit directly)
...
- [`CustomThemeConfig`](src/ui/themes/types.ts#L14) (interface)
- [`Theme`](src/ui/themes/types.ts#L32) (interface)
- [`ThemeRegistry`](src/ui/themes/types.ts#L41) (interface)
- [`ThemeBuilderOptions`](src/ui/themes/types.ts#L48) (interface)
- [`ZIndexToken`](src/ui/tokens/z-index.ts#L19) (interface)
...
```bash
# Start Storybook to test component
npm run storybook

# Or run tests to see failure
npm run test:watch
```

### 2. Identify Root Cause

- Check component source in `src/ui/{category}/{ComponentName}/`
- Review related hooks in `src/ui/hooks/` or component-specific hooks
- Check providers if issue involves context/state
- Review design tokens if styling issue
- Check validation scripts: `npm run validate:all`
...
- Compound component pattern broken
- Props not forwarded correctly
- Children not handled properly

### 4. Implement Fix

- Follow existing patterns in similar components
- Use design tokens from `src/ui/tokens/`
...
2. Create new branch: `git checkout -b fix/alternative-approach`
...
- Use custom error classes from `src/ui/shared/errors/RegistryErrors.ts`
...
- Storybook addons (a11y, measure, outline)
- Vitest debug mode: `npm run test -- --inspect`

## Hand-off Notes

After completing a bug fix, document:

- **Root Cause**: What caused the bug
- **Solution**: How it was fixed
- **Tests Added**: Regression tests created
- **Related Components**: Other components that might be affected
- **Breaking Changes**: Any API changes (use `BREAKING CHANGE:` in commit)
- **Follow-up**: Additional improvements or related issues to address
